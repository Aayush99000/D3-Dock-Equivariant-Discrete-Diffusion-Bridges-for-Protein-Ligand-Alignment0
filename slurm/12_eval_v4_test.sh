#!/bin/bash
#SBATCH --job-name=d3dock_v4_test
...
#SBATCH --output=/scratch/katoch.aa/d3dock/logs/12_eval_v4_test_%j.out
#SBATCH --error=/scratch/katoch.aa/d3dock/logs/12_eval_v4_test_%j.err
...
OUT=$BASE/outputs/eval_test_v4_T200

mkdir -p $OUT

echo "[$(date)] Evaluating v4 ep200 on TEST split at T=200"
...
python scripts/validate_test_d3dock.py \
    --checkpoint      $CKPT \
    --input-csv       $SPLITS/test_systems.csv \
    --structures-dir  $STRUCTS \
    --crop-dir        $CROPS \
    --output-dir      $OUT \
    --T               200

echo "[$(date)] Test eval complete."
echo "=== SUMMARY ==="
